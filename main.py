...
class MDP:
    ...
    def new_utility(self, x, y):
        ...
        for (dx, dy) in self.vertical_change:
            ...
            for (dx_w, dy_w) in self.horizontal_change:
                value += self.alternate_probability * self.get_utility(x, y, x + dx_w, y + dy_w)
    ...
    def value_iteration(self):
        ...
        print_board_mysql_style(
            self.utility_map, self.rows_num, self.cols_num, 18)
                ...
                for j in range(self.cols_num):
                    if (i, j) not in self.end_states and self.will_go(i, j):
                        ...
                        if self.utility_map[i][j] != 0:
                            frac_change = (new_utility_map[i][j] - self.utility_map[i][j])/self.utility_map[i][j]
...
if __name__ == '__main__':
    ...
    mdp = MDP(board, E, W, unit_step_reward, 0.8, 0.1, 0.01, 0.99, n, m)
